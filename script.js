(function () {
  ...
  // state
  const state = { name: '', country: '', goal: '', count: 1, domains: [''], desc: [''], domainCheckIndex: 0 };
  ...
  if (inputName && btnNameNext) {
    ...
    btnNameNext.addEventListener('click', () => show('#step-country'));
  } else {
  ...
  // --- Country step ---
  if (qs('#btn-country-back')) qs('#btn-country-back').addEventListener('click', () => show('#step-name'));
  ...
  // --- Cambiar flujo: después de dominios, comprobar existencia ---
  if (btnDomainsNext)
    btnDomainsNext.addEventListener('click', () => {
      state.domainCheckIndex = 0;
      showDomainCheckStep();
    });

  // --- Nueva lógica: comprobación de dominio ---
  const domainCheckLink = qs('#domain-check-link');
  const btnDomainCheckOpen = qs('#btn-domain-check-open');
  const domainCheckConfirm = qs('#domain-check-confirm');
  const btnDomainCheckYes = qs('#btn-domain-check-yes');
  const btnDomainCheckNo = qs('#btn-domain-check-no');

  function showDomainCheckStep() {
    // Mostrar el dominio actual a comprobar
    const idx = state.domainCheckIndex || 0;
    const dom = state.domains[idx] || '';
    if (!dom) { show('#step-domains'); return; }
    show('#step-domain-check');
    if (domainCheckLink) {
      domainCheckLink.innerHTML = `<strong>Dominio:</strong> <span style="color:#7dd3fc">https://${dom}.netlify.app</span>`;
    }
    if (domainCheckConfirm) domainCheckConfirm.classList.add('hidden');
  }

  if (btnDomainCheckOpen) {
    btnDomainCheckOpen.addEventListener('click', () => {
      const idx = state.domainCheckIndex || 0;
      const dom = state.domains[idx] || '';
      if (!dom) return;
      const url = `https://${dom}.netlify.app`;
      window.open(url, '_blank');
      // Mostrar confirmación al volver
      setTimeout(() => {
        if (domainCheckConfirm) domainCheckConfirm.classList.remove('hidden');
      }, 300);
    });
  }

  // Botón "No existe" (avanza al siguiente dominio o a descripciones)
  if (btnDomainCheckYes) {
    btnDomainCheckYes.addEventListener('click', () => {
      if (state.domainCheckIndex + 1 < state.count) {
        state.domainCheckIndex++;
        showDomainCheckStep();
      } else {
        renderDescInputs();
        show('#step-desc');
  ...
  // Botón "Sí, cambiar dominio" (regresa a editar el dominio actual)
  if (btnDomainCheckNo) {
    btnDomainCheckNo.addEventListener('click', () => {
      show('#step-domains');
      setTimeout(() => {
        const rows = domainsContainer ? domainsContainer.querySelectorAll('input') : [];
        if (rows && rows[state.domainCheckIndex]) rows[state.domainCheckIndex].focus();
      }, 200);
    });
  }
